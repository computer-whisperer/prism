...
**Task #59 (multi-output + multi-GPU foundation) complete.** `prism run` opens every connected card, brings up all connected connectors across both GPUs as independent `OutputContext`s, pumps vblank-driven double-buffered scanout on each at its native rate, advertises one wl_output (+ xdg-output) per OutputContext on a horizontal-stack logical layout, and routes each client surface to exactly one output via center-containment. End-to-end verified on TTY against `scripts/tty-test.sh` + `prism-shmtest`: 7 outputs (5 on card0/Vega20, 2 on card1/Navi21) brought up and pumped frames at 60–90 Hz; shmtest at logical `(0,0)` routes to DP-1 (the leftmost output, `wl_surface.enter` confirmed in logs); shutdown ~1.4s clean.

Renderer CPU cost in the steady-state present path: ~60 µs/frame per output. dmabuf and shm client buffers are both imported/uploaded on every registered GPU at commit time (any output's render path can sample without GPU↔GPU copies); per-GPU shm narrowing — upload only to the GPUs hosting the surface, derived from its `current_output` — is a follow-on optimization once we have a workload that needs it.

With #59 done, the foundation is in place for the HDR / calibration work to land per-output: EDID parsing populates `OutputConfig` defaults, HDR signaling (HDR_OUTPUT_METADATA + Colorspace) is a per-output atomic-commit property, fp16 scanout swaps `OutputConfig.depth/vk_format`, per-display calibration matrices ride along in `EncodeConfig`. None of these need further structural work; they slot into the existing per-output config carrier.
...
| 59 | Multi-output + multi-GPU foundation | ✅ | All 5 sub-steps done: structural split (59.1), multi-output (59.2), multi-GPU + per-GPU client buffer replication (59.3), wl_output + xdg-output advertisement (59.4), per-output element mapping via center-containment + `wl_surface.enter`/`.leave` (59.5). |
...
- **59.5 ✅ — per-output element mapping.** `SurfacePlacementSlot { logical_pos, current_output }` added to surface `data_map` alongside `SurfaceTexSlot`. Default `logical_pos = (0, 0)` for every new surface until a config / layout layer assigns real coordinates. `process_surface_buffer` computes the surface's center after each new-buffer commit; `PrismState::output_containing(point)` finds the wl_output whose advertised geometry contains it. On transition we dispatch `wl_surface.enter` / `.leave` on the appropriate smithay `Output`s (executed outside the `with_states` callback to avoid nesting smithay's surface bookkeeping under our SurfaceData lock). `present_for_crtc` filters toplevels by `current_output == this_output.connector_name`. Per-GPU shm upload narrowing (use the output assignment to upload only to the GPUs hosting the surface, not every GPU) is a follow-on optimization.
