//! VRR-aware per-output frame pacing.
//!
//! Ported verbatim from niri/src/frame_clock.rs. Owns the most recent
//! presentation timestamp for a single CRTC and projects the next vblank
//! the compositor should aim for.
//!
//! Used by the render loop to:
//!   - decide what `target_presentation_time` to hand to clients on the
//!     frame-callback dispatch (so they animate against the right clock);
//!   - early-vblank handling (some kernels fire vblank a hair before
//!     the actual scanout edge — we forward the clock by one refresh
//!     interval to keep monotonic);
//!   - VRR: when more than one refresh interval has elapsed since the
//!     last presentation, fire immediately rather than queue at the
//!     next stride.
