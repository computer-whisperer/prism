//! `FloatingSpace<W>` — niri's floating-window layer.
//!
//! Full state-machine port from niri/src/layout/floating.rs.
//! Render emission stubbed in the same shape as [`super::scrolling`]:
//! the `FloatingSpaceRenderElement<R>` enum is dropped, the render
//! method emits a flat `Vec<RenderEl>` via per-tile `Tile::render`.
...
use super::{
    ConfigureIntent, InteractiveResize, InteractiveResizeData, LayoutElement, Options, RemovedTile,
    ResolvedSize, SizeFrac,
};
...
// niri defines `FloatingSpaceRenderElement<R>` via the
// `niri_render_elements!` macro (Tile + ClosingWindow variants).
...
impl<W: LayoutElement> FloatingSpace<W> {
    ...
    /// Emit this space's draw calls into `out`. See
    /// [`super::scrolling::ScrollingSpace::render`] for the render-side
    /// deficit context — same shape, same stubs.
    pub fn render(
        &self,
