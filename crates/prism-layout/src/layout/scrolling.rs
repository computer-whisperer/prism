//! `ScrollingSpace<W>` — niri's scrollable-tiling implementation.
//!
//! Full state-machine port from niri/src/layout/scrolling.rs. View
//! offset, view gestures (swipe + touchpad pinch), column / tile
//! geometry, all the column-insert / focus-shift / fullscreen
//! coordination is preserved. Render emission stripped following the
//! same pattern as `tile.rs` / `mapped.rs`: the niri_render_elements!
//! macro for `ScrollingSpaceRenderElement` is dropped, the
//! `render_above_top_layer` / `render_elements` methods stub to
//! `Vec<RenderEl>` emission walking each tile via `Tile::render`.
//!
//! Outstanding shape mismatch: niri's `RemovedTile` carries
//! `width: ColumnWidth`. The type `ColumnWidth` is defined in this
//! file, so the parent mod's `RemovedTile<W>` declaration references
//! it as `scrolling::ColumnWidth` — that's why this module declares
//! `pub use` for the enum.
...
use super::tab_indicator::{TabIndicator, TabInfo};
use super::tile::{Tile, TileRenderSnapshot};
use super::{
    ConfigureIntent, HitType, InsertPosition, InteractiveResize, InteractiveResizeData,
    LayoutElement, Options, RemovedTile, ResolvedSize, SizingMode,
};
...
// niri defines a `ScrollingSpaceRenderElement` enum here via the
// `niri_render_elements!` macro (Tile + ClosingWindow + TabIndicator).
// Prism's render path emits a flat `Vec<RenderEl>` instead, so no
// enum is needed.
...
impl<W: LayoutElement> ScrollingSpace<W> {
    ...
    fn activate_column_with_anim_config(&mut self, idx: usize, config: prism_config::Animation) {
    ...
    pub fn add_tile(
        ...
        width: ColumnWidth,
        is_full_width: bool,
        anim_config: Option<prism_config::Animation>,
    ) {
    ...
    /// active-tile focus-ring) is preserved verbatim.
    pub fn render(
        &self,
        focus_ring: bool,
...
impl ViewGesture {
    fn animate_from(&mut self, from: f64, clock: Clock, config: prism_config::Animation) {
...
fn compute_toplevel_bounds(
    border_config: prism_config::Border,
