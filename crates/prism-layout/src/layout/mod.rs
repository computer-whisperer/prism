...
use self::scrolling::ScrollDirection;
use self::workspace::{Workspace, WorkspaceAddWindowTarget};
...
// niri's `mod tests` block is deferred — the test corpus references
// render-helpers types we don't carry; it'll come across once
// prism's render-helper equivalents land.
...
/// close animations. Niri stores a baked GLES texture here; prism
/// stubs to `{ size }` until the offscreen-FBO pipeline lands. The
/// surrounding state machine still consults `Option<...>` to gate
/// the resize-crossfade trigger.
#[derive(Debug)]
pub struct LayoutElementRenderSnapshot {
    pub size: smithay::utils::Size<f64, smithay::utils::Logical>,
}

/// Stable per-workspace ID.
...
/// Where a window goes when dropped on a workspace.
...
pub struct InsertHint {
    ...
    pub corner_radius: prism_config::CornerRadius,
}

/// In-progress interactive resize.
#[derive(Debug)]
...
struct InteractiveMoveData<W: LayoutElement> {
    ...
    pub(self) output_config: Option<prism_config::LayoutPart>,
    ...
    pub(self) workspace_config: Option<(WorkspaceId, prism_config::LayoutPart)>,
}
...
// `InteractiveResizeData` and `ConfigureIntent` live in
// `element.rs` (re-exported above). Niri kept them inline next to
// the trait; we keep them in the same module as the trait they
// belong to.
...
// `impl SizingMode` lives in `element.rs` next to the type def.
...
impl Options {
    ...
    fn with_merged_layout(mut self, part: Option<&prism_config::LayoutPart>) -> Self {
...
impl<W: LayoutElement> Layout<W> {
    ...
    /// Snapshot the window's render before unmap. Niri takes a
    /// `&mut GlesRenderer` + xray state; ours is renderer-free
    /// (only the geometry is retained — see
    /// [`LayoutElementRenderSnapshot`]).
    pub fn store_unmap_snapshot(&mut self, window: &W::Id) {
        let zoom = self.overview_zoom();
            ...
            MonitorSet::Normal { monitors, .. } => {
                for mon in monitors {
                    for (ws, _geo) in mon.workspaces_with_render_geo_mut(false) {
                        if ws.has_window(window) {
                            ws.store_unmap_snapshot_if_empty(window);
                            return;
                        }
                    }
                }
    ...
    /// Trigger the close animation for `window`. Niri takes a
    /// `&mut GlesRenderer` to bake the snapshot texture; ours is
    /// renderer-free (see [`LayoutElementRenderSnapshot`]).
...
// `compute_overview_zoom` lives near the scaffold types at the top
// of this module so monitor.rs could reach it before this file's
// Layout<W> impl landed.
