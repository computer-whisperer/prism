//! `Monitor<W>` — one output's workspace stack + workspace-switch UI.
//!
//! Full state-machine port from niri/src/layout/monitor.rs. Render
//! emission stripped following the established pattern — see
//! [`super::tile`] for the deficit catalogue.
...
use super::insert_hint_element::InsertHintElement;
use super::scrolling::{Column, ColumnWidth};
use super::tile::Tile;
use super::workspace::{compute_working_area, Workspace, WorkspaceAddWindowTarget};
use super::{
    compute_overview_zoom, ActivateWindow, HitType, InsertHint, InsertPosition, InsertWorkspace,
    LayoutElement, Options, OutputId, WorkspaceId,
};
...
// `InsertPosition`, `InsertWorkspace`, `InsertHint` now live in
// `super::` (layout/mod.rs) so `scrolling.rs` could reference
// `InsertPosition` before this module landed. See the scaffold
// block in `layout/mod.rs`.
...
// niri defines `MonitorInnerRenderElement<R>` and the
// `MonitorRenderElement<R>` type alias here (Crop + Relocate +
// Rescale + WorkspaceRenderElement composition). Prism emits a flat
// `Vec<RenderEl>`; the crop/relocate/rescale wrappers are applied
// inside the render walk via per-element `project` closures.
...
impl<W: LayoutElement> Monitor<W> {
    ...
    pub fn activate_workspace_with_anim_config(
        &mut self,
        idx: usize,
        config: Option<prism_config::Animation>,
    ) {
    ...
    /// Render the insert-hint element shown between workspaces during
    /// drag-into-new-workspace. Niri composes 5 render-helpers
    ...
    /// stack, per-workspace cropping during workspace-switch.
