//! `Workspace<W>` — one workspace = one ScrollingSpace + one FloatingSpace.
//!
//! Full state-machine port from niri/src/layout/workspace.rs.
//! Render emission stripped following the same pattern as the prior
//! port chunks — see [`super::tile`] for the deficit catalogue.
...
use super::scrolling::{Column, ColumnWidth, ScrollDirection, ScrollingSpace};
...
pub struct Workspace<W: LayoutElement> {
    ...
    /// This workspace's background colour.
    ///
    /// Niri caches a `SolidColorBuffer` here; prism emits the
...
// `OutputId`, `WorkspaceId`, `InteractiveResize<W>`, `ResolvedSize`
// are now defined in `super::` (layout/mod.rs) since they're shared
// between `scrolling.rs`, `floating.rs`, and this module — niri kept
// them inside `workspace.rs` with `pub(super)` visibility, but our
// scaffolding lifted them up so each downstream port could compile
// independently. See the scaffold block in `layout/mod.rs`.

// niri defines `WorkspaceRenderElement<R>` via the
// `niri_render_elements!` macro (Scrolling + Floating variants).
// Prism's render path emits flat `Vec<RenderEl>` directly, so the
// wrapper enum is dropped.
...
// `OutputId::new` lives on the type in `super::` (layout/mod.rs).
...
impl<W: LayoutElement> Workspace<W> {
    ...
    pub fn update_config(&mut self, base_options: Rc<Options>) {
        ...
        self.background_color = options.layout.background_color;

        self.base_options = base_options;
        self.options = options;
    }

    pub fn update_layout_config(&mut self, layout_config: Option<prism_config::LayoutPart>) {
    ...
    fn set_view_size(
        ...
        // niri resizes its `background_buffer` here; ours is just a
        // colour and is sized at render time from `self.view_size`.
        let _ = size;
    ...
    /// Emit the scrolling layer's draw calls into `out`.
    /// See [`super::scrolling::ScrollingSpace::render`] for deficit.
    ...
    /// Capture the about-to-unmap tile's render into a snapshot, so
    /// the close animation has something to crossfade against. Niri
    /// takes &mut GlesRenderer + xray state here; our stub keeps the
    /// state-machine call (`Tile::store_unmap_snapshot_if_empty`)
    /// but no renderer / xray.
    pub fn store_unmap_snapshot_if_empty(&mut self, window: &W::Id) {
    ...
    pub fn layout_config(&self) -> Option<&prism_config::LayoutPart> {
    ...
    pub fn verify_invariants(&self, move_win_id: Option<&W::Id>) {
        ...
        // niri also asserts `background_buffer.size() == view_size`
        // here; our `background_color` is renderer-free so the size
        // invariant is implicit in `view_size`.
        assert_eq!(
...
fn compute_workspace_shadow_config(
    config: prism_config::WorkspaceShadow,
    view_size: Size<f64, Logical>,
) -> prism_config::Shadow {
    ...
    let mut config = prism_config::Shadow::from(config);
