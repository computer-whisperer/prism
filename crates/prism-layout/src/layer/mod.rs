use prism_config::layer_rule::{LayerRule, Match};
use prism_config::utils::MergeWith as _;
use prism_config::{BackgroundEffect, BlockOutFrom, CornerRadius, ResolvedPopupsRules, ShadowRule};
...
fn surface_matches(surface: &LayerSurface, m: &Match) -> bool {
    ...
    if let Some(layer) = m.layer {
        let surface_layer = match surface.layer() {
            Layer::Background => prism_ipc::Layer::Background,
            Layer::Bottom => prism_ipc::Layer::Bottom,
            Layer::Top => prism_ipc::Layer::Top,
            Layer::Overlay => prism_ipc::Layer::Overlay,
        };
