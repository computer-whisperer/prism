//! Cursor theme loading + image cache.
//!
//! Ported from niri/src/cursor.rs. The XCursor parsing + theme loading +
//! `CursorImageStatus` plumbing all comes across verbatim. The texture
//! cache changed: niri caches uploaded `MemoryRenderBuffer`s (a smithay
//! GLES helper), but prism's Vulkan path uploads from raw ARGB8888
//! pixels directly — so `CursorTextureCache` here stores
//! `CursorImageFrame` (pixels + dimensions + scale + transform), and the
//! Vulkan renderer turns those into VkImages downstream.
...
/// A single decoded cursor frame ready for upload to a GPU texture.
///
/// Replaces niri's `MemoryRenderBuffer` (a smithay GLES helper) with the
/// raw form the Vulkan renderer wants: ARGB8888 pixels, dimensions, the
/// scale this frame was selected for, and a transform.
#[derive(Clone)]
pub struct CursorImageFrame {
    pub pixels_rgba: Rc<Vec<u8>>,
    pub width: u32,
    pub height: u32,
    pub scale: i32,
...
type TextureCache = HashMap<(CursorIcon, i32), Vec<CursorImageFrame>>;
        ...
        self.cache
            ...
            .or_insert_with(|| {
                cursor
                    .frames()
                    .iter()
                    .map(|frame| CursorImageFrame {
                        pixels_rgba: Rc::new(frame.pixels_rgba.clone()),
                        width: frame.width,
                        height: frame.height,
                        scale,
